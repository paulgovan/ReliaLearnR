...
Reliability, Availability, and Maintainability may sound like similar terms, but they actually represent different concepts. 

**Reliability** is the ability of an item to not break down over time (e.g. a car that doesn't have to go to the shop very often). 

**Availability** is the ability of an item to be available for use (e.g. a car that's waiting patiently in the garage).

**Maintainability** is the ability of an item to be maintained efficiently (e.g. a car that's quick to fix if it has a problem).

That being said, all 3 concepts are inter-related. For example, a car that's not very reliable is probably not available very often. A car that's also difficult to maintain is probably available even less. 
