...
quiz(caption = "Quiz: NHPP Analysis",
  question("In a Crow-AMSAA plot, what does a curve that bends downward (concave shape) indicate?",
    ...
    answer("The failure rate is constant",
           message = "A straight line on the cumulative failures plot would indicate a constant rate (HPP), not a concave curve."),
    answer("The failure rate is decreasing over time", correct = TRUE,
           message = "A concave (downward-bending) cumulative failures curve means β < 1: each successive failure takes longer to arrive, so the system is improving."),
    random_answer_order = TRUE,
    allow_retry = TRUE
  ),
  question("For the pump fleet example, which statement best characterizes the system based on the nhpp() fit?",
    ...
    answer("The pumps show a perfectly constant failure rate (HPP)",
           message = "A straight cumulative failures line would indicate HPP. The concave shape indicates a trend."),
  ...
  question("In nhpp(), what does the event argument represent when fitting interval-aggregated data?",
    answer("The failure times of individual events",
           message = "Individual failure times can be passed via the time argument with event = NULL. The event argument accepts counts per interval."),
    answer("The number of failures observed in each time interval", correct = TRUE,
           message = "When event is provided, nhpp() treats each entry in time as the end of an interval and the corresponding entry in event as the failure count for that interval."),
    answer("A binary indicator of whether the system was censored",
           message = "Censoring indicators are used by mcf() and exposure(). In nhpp(), event holds failure counts per interval."),
    random_answer_order = TRUE,
    allow_retry = TRUE
  )
)
