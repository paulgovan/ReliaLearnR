...
Welcome to the tutorial on **Life Data Analysis**! In this tutorial, you will learn about the basics of life data analysis and how to apply it to various fields, including automotive, aerospace, and many others.
...
Let's add right censored data to the previous example. In addition to the 5 machines that failed, add suspensions for 3 machines that did not fail (right censored) at times 100, 45, and 10 respectively. Then use the `MLEw2p` function to fit a Weibull to the data using the MLE method.
...
Let's rerun the right censored example and compare the MRR and MLE methods using the `wblr` function.
...
Let's rerun a previous example as a WeiBayes model with a Beta of 2. First,  create the failure and suspension data. Then fit the WeiBayes model and plot the results.
...
Let's run this example as a 3P Weibull. First, create the failure data. Then fit the 3P Weibull model and plot the results.
...
A part or machine may have multiple different failure modes, where each failure mode has a different distribution of time to failure. A failure mode is a specific way in which a part or machine can fail  (e.g. corrosion, fatigue, overload). 
...
The overall fit has a beta of approximately 1, which indicates random failures. However, we know that the data actually contains 3 different failure modes.
...
Let's start by rerunning the first example and building a contour plot. First, create the failure data. 
Then fit the Weibull model and build the contour plot at a probability level of 90%.
...
The blue, red, and orange contours represent the 3 different failure modes. 
