...
Notice how the previous example contained groups in the `qty` column. Let's look at the inspection data again. The knitr::kable function below displays the inspection data in a table format, and the head function shows the first few rows of the data.
...
To illustrate the differences between MRR and MLE, let's rerun the right censored example and compare the MRR and MLE methods using the `wblr` function. In the first step, fit the MRR and MLE models using the `wblr.fit` function with the `method.fit` argument set to "rr" for MRR and "mle" for MLE. Finally, use the `plot.wblr` function to create a multi-plot comparing both models.
