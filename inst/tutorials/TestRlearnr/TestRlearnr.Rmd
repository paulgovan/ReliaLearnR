...
*Hint: the Duane Plot does not necessarily provide information on the type of failure mode.*
...
```{r duane3, echo=FALSE}
question("In this example, what is the approximate cumulative MTBF at time 100?",
  answer("200"),
  answer("100"),
  answer("120"),
...
```{r rga2, echo=FALSE}
question("How would you describe the reliability of this system?",
  answer("The reliability of the system is mproving"),
  answer("The reliability of the system is worsening", correct = TRUE),
...
For this example, we will use the built in data set `MeekerData`. First, load the data set and view the first few samples.
...
Finally, use the `alt.make` function to fit an Arrhenius-Weibull model to the data.
...
For this example, we will use the built in data set `NelsonData`. First, load the data set and view the type of data.
...
Finally, use the `alt.make` function to fit an Power-Weibull model to the data.
...
```{r pwr2, echo=FALSE}
question("How should you interpret a subset that is plotted to the right of another set (e.g. set 2 in green vs set 3 in red)?",
  answer("The set to the right has a lower unreliability than the one to the left", correct = TRUE),
  answer("The set to the right a higher unreliability than the one to the left"),
  answer("The set to the right has a lower reliability than the one to the left"),
  answer("The set to the right has a higher reliability than the one to the left", correct = TRUE),
  random_answer_order = TRUE
)
```
...
To run a parallel model, we can use the `alt.parallel` function

For the Meekder data example:
```{r}
prlobj <- alt.parallel(arrobj)
```

For the Nelson data example:
```{r}
