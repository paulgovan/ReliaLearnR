...
where $R(t)$ is the cumulative probability of *survival* prior to time $t$, $\beta$ is the shape parameter, and $\eta$ is the scale parameter.
...
Both methods fit the same data. Slight differences in $\beta$ and $\eta$ are visible — the better fit can be determined objectively using the Anderson-Darling statistic.
